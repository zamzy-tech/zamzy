...
const logStream = fs.createWriteStream(path.join(logDir, 'console.log'), { flags: 'a' });
...
console.error = function(...args) {
    const formatted = args.map(arg => typeof arg === 'object' ? JSON.stringify(arg) : arg).join(' ');
    logStream.write(`[ERR ${new Date().toISOString()}] ${formatted}\n`);
    originalError.apply(console, args);
};

// --- Single Process PID Lock to prevent duplicate session logout conflict loop ---
const pidFile = path.join(logDir, 'whatsapp.pid');
try {
    if (fs.existsSync(pidFile)) {
        const oldPid = parseInt(fs.readFileSync(pidFile, 'utf8'), 10);
        if (oldPid && oldPid !== process.pid) {
            try {
                process.kill(oldPid, 0); // Check if process is running
                console.log(`[WhatsApp Service] Old conflicting process ${oldPid} is still active. Terminating it...`);
                process.kill(oldPid, 'SIGTERM');
                // Cross-platform sync sleep for 1 second to release ports/files
                const delay = Date.now();
                while (Date.now() - delay < 1000) {}
            } catch (e) {
    ...
    console.error('[WhatsApp Service] PID lock check failed:', err.message);
}
...
function debugLog(text) {
    try {
        const logPath = path.join(__dirname, 'tmp', 'chatbot_debug.log');
...
async function initializeSession(sessionId) {
    ...
    try {
        ...
        let version = [6, 7, 7];
        try {
            console.log(`[WhatsApp Service] [${sessionId}] Fetching latest Baileys version...`);
            const versionPromise = fetchLatestBaileysVersion();
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 2000));
            const latest = await Promise.race([versionPromise, timeoutPromise]);
            version = latest.version;
            console.log(`[WhatsApp Service] [${sessionId}] Latest Baileys version: ${version.join('.')}`);
        } catch (vErr) {
            console.log(`[WhatsApp Service] [${sessionId}] Using default Baileys version [6, 7, 7] due to:`, vErr.message);
        }

        // Set safety init timeout (30 seconds)
        if (sessionObj.initTimeout) {
            clearTimeout(sessionObj.initTimeout);
        }
        sessionObj.initTimeout = setTimeout(() => {
            if (sessionObj.connectionStatus === 'INITIALIZING') {
                console.error(`[WhatsApp Service] [${sessionId}] Initialization timed out (stuck in INITIALIZING for 30s). Resetting connection.`);
                sessionObj.connectionStatus = 'DISCONNECTED';
                try {
                    if (sessionObj.sock) {
                        sessionObj.sock.end(new Error('Initialization Timeout'));
                    }
                } catch (sockErr) {
                    console.error(`[WhatsApp Service] Error closing stuck socket for ${sessionId}:`, sockErr.message);
                }
                sessionObj.sock = null;
                sessionObj.latestQr = null;
                sessionObj.latestQrBase64 = null;
            }
            sessionObj.initTimeout = null;
        }, 30000);
        ...
        sock.ev.on('chats.set', ({ chats }) => {
            if (chats && chats.length) {
                sessionObj.chatsCount = Math.max(sessionObj.chatsCount || 0, chats.length);
            }
        });
        ...
        sock.ev.on('contacts.set', ({ contacts }) => {
            if (contacts && contacts.length) {
                sessionObj.contactsCount = Math.max(sessionObj.contactsCount || 0, contacts.length);
            }
        });
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                const linkedNumber = rawUser.split(':')[0] || rawUser;

                // Check if this number is already linked to ANOTHER session/slot
                let duplicateSessionId = null;
                for (const id in sessions) {
                    if (id !== sessionId && sessions[id].linkedNumber === linkedNumber && sessions[id].connectionStatus === 'CONNECTED') {
                ...
                if (duplicateSessionId) {
                    console.log(`[WhatsApp Service] Session ${sessionId} connected to +${linkedNumber}, but this number is already linked to session ${duplicateSessionId}. Force disconnecting to prevent conflict...`);
                    sessionObj.connectionStatus = 'DISCONNECTED';
                    sessionObj.linkedNumber = null;
                    try {
                        await sock.logout();
                    } catch (logoutErr) {
                        console.error(`[WhatsApp Service] Error logging out duplicate session:`, logoutErr.message);
                    }
                    cleanupSessionDir(sessionId);
                    sessionObj.lastError = `This WhatsApp number (+${linkedNumber}) is already linked to another device slot. Please use a different WhatsApp account or unlink it from the other slot first.`;
                    return;
                }

                sessionObj.linkedNumber = linkedNumber;
            ...
            if (connection === 'close') {
                ...
                // Do NOT reconnect if logged out (401) or if connection was replaced by another device/session (440)
                ...
                console.log(`[WhatsApp Service] Connection closed for ${sessionId}. StatusCode: ${statusCode}. Reconnecting: ${shouldReconnect}. Error:`, lastDisconnect?.error);
                ...
                if (shouldReconnect) {
                    if (sessionObj.reconnectTimeout) {
                        clearTimeout(sessionObj.reconnectTimeout);
                    }
                    sessionObj.reconnectTimeout = setTimeout(() => {
                        sessionObj.reconnectTimeout = null;
                        initializeSession(sessionId);
                    }, 5000);
                } else {
                    if (sessionObj.reconnectTimeout) {
                    ...
                    if (isReplaced) {
                        sessionObj.lastError = 'This device connection was replaced by another active session or device connection. Please re-authenticate if necessary.';
                    } else {
                        sessionObj.lastError = 'Logged out from WhatsApp. Please scan the QR code to connect again.';
                    }
                    
                    console.log(`[WhatsApp Service] Session ${sessionId} closed permanently. Clearing auth files...`);
                    cleanupSessionDir(sessionId);
        ...
        sock.ev.on('messages.upsert', async (m) => {
            debugLog(`messages.upsert event received of type: ${m.type}`);
            if (m.type !== 'notify') return;
            
            for (const msg of m.messages) {
                if (msg.key.fromMe) {
                    debugLog(`Ignoring message fromMe`);
                    continue;
                }
                
                const senderJid = msg.key.remoteJid;
                if (!senderJid) {
                    debugLog(`Ignoring message: missing remoteJid`);
                    continue;
                }
                if (senderJid.endsWith('@g.us')) {
                    debugLog(`Ignoring group message from ${senderJid}`);
                    continue;
                }
                ...
                const messageText = msg.message?.conversation || 
                                    ...
                                    '';
                debugLog(`Parsed message content from ${senderJid}: "${messageText}"`);
                if (!messageText.trim()) {
                    debugLog(`Ignoring message: text content is empty or unhandled type`);
                    continue;
                }
                ...
                try {
                    ...
                    debugLog(`Sending POST query to webhook: ${webhookUrl} (sender: ${cleanSender}, message: "${messageText}")`);
                    const resJson = await postToWebhook(webhookUrl, {
                        sender: cleanSender,
                        message: messageText,
                        session_id: sessionId
                    });
                    debugLog(`Webhook Response: ${JSON.stringify(resJson)}`);
                    
                    if (resJson && resJson.reply) {
                        debugLog(`Sending auto-reply to ${senderJid}`);
                        
                        let replyText = resJson.reply;
                        
                        // Format buttons as neat bracketed tags at the bottom
                        if (resJson.buttons && Array.isArray(resJson.buttons) && resJson.buttons.length > 0) {
                            const btnTextList = resJson.buttons.map(b => `[ ${b.text} ]`).join("   ");
                            replyText += "\n\n" + btnTextList;
                        }
                        
                        if (resJson.image_url) {
                            try {
                                debugLog(`Downloading image: ${resJson.image_url}`);
                                const imageBuffer = await downloadBuffer(resJson.image_url);
                                await sock.sendMessage(senderJid, { 
                                    image: imageBuffer, 
                                    caption: replyText 
                                });
                                debugLog(`Image reply sent!`);
                            } catch (imgErr) {
                                debugLog(`Image download failed: ${imgErr.message}. Falling back to text.`);
                                await sock.sendMessage(senderJid, { text: replyText });
                                debugLog(`Text reply sent (fallback)!`);
                            }
                        } else {
                            await sock.sendMessage(senderJid, { text: replyText });
                            debugLog(`Text reply sent!`);
                        }
                    } else {
                        debugLog(`No reply configured or webhook returned empty reply`);
                    }
                } catch (err) {
                    debugLog(`Webhook query failed: ${err.message}`);
                }
            }
        });
...
app.get('/status', async (req, res) => {
    ...
    res.json({
        ...
        chat_count: session.chatsCount || 0,
        contact_count: session.contactsCount || 0,
...
app.get('/groups', async (req, res) => {
    ...
    if (!session || session.connectionStatus !== 'CONNECTED' || !session.sock) {
        return res.status(503).json({ success: false, error: 'WhatsApp session is not connected.' });
    }
    try {
        const groups = await session.sock.groupFetchAllParticipating();
        const groupList = Object.keys(groups).map((jid) => ({
            jid: jid,
