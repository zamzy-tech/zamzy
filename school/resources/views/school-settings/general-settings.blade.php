                    ...
                    <div class="card-body">
                        <form class="create-form-without-reset" action="{{ route('school-settings.store') }}" method="POST" novalidate="novalidate" enctype="multipart/form-data">
                            ...
                            <div class="border border-secondary rounded-lg my-4 mx-1">
                                <div class="col-md-12 mt-3"><h4>{{__("UPI / QR Code Settings (Manual Payment)")}}</h4></div>
                                <div class="col-12 mb-3">
                                    <hr class="mt-0">
                                </div>
                                <div class="row my-4 mx-1">
                                    <div class="form-group col-md-6 col-sm-12">
                                        <label for="upi_id">{{ __('UPI ID') }}</label>
                                        <input type="text" name="upi_id" id="upi_id" class="form-control" placeholder="example@upi" value="{{ $settings['upi_id'] ?? '' }}"/>
                                    </div>
                                    <div class="form-group col-md-6 col-sm-12">
                                        <label for="upi_qr_code">{{ __('UPI QR Code') }}</label>
                                        <input type="file" name="upi_qr_code" class="file-upload-default"/>
                                        <div class="input-group col-xs-12">
                                            <input type="text" class="form-control file-upload-info" id="upi_qr_code_info" disabled="" placeholder="{{ __('UPI QR Code') }}"/>
                                            <span class="input-group-append">
                                                <button class="file-upload-browse btn btn-theme" type="button">{{ __('upload') }}</button>
                                            </span>
                                            <div class="col-md-12 mt-2">
                                                @if(isset($settings['upi_qr_code']) && !empty($settings['upi_qr_code']))
                                                    <img height="150px" src='{{ $settings['upi_qr_code'] }}' alt="UPI QR Code">
                                                @endif
                                            </div>
                                        </div>
                                    </div>
