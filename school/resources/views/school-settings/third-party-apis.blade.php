                ...
                <div class="card">
                    <div class="custom-card-body">
                        <form id="formdata" class="create-form-without-reset" action="{{ route('school-settings.third-party.update') }}" method="POST" novalidate="novalidate" enctype="multipart/form-data">
                            ...
                            {{-- End google_recaptcha --}}

                            {{-- WhatsApp Settings --}}
                            <div class="border border-secondary rounded-lg my-4 mx-1">
                                <div class="col-md-12 mt-3">
                                    <h4>{{ __('WhatsApp Notifications') }}</h4>
                                </div>
                                <div class="col-12 mb-3">
                                    <hr class="mt-0">
                                </div>
                                <div class="row my-4 mx-1">
                                    <div class="form-group col-md-4 col-sm-12">
                                        <label for="whatsapp_status">{{ __('Enable WhatsApp Notifications') }}</label>
                                        <select name="whatsapp_status" id="whatsapp_status" class="form-control">
                                            <option value="0" {{ ($schoolSettings['whatsapp_status'] ?? 1) == 0 ? 'selected' : '' }}>{{ __('Disabled') }}</option>
                                            <option value="1" {{ ($schoolSettings['whatsapp_status'] ?? 1) == 1 ? 'selected' : '' }}>{{ __('Enabled') }}</option>
                                        </select>
                                    </div>
                                </div>

                                <div id="whatsapp_connection_section" class="row my-4 mx-1" style="display: {{ ($schoolSettings['whatsapp_status'] ?? 1) == 1 ? 'flex' : 'none' }}; align-items: center;">
                                    <div class="col-md-12 mb-3">
                                        <h5>{{ __('WhatsApp Connection Status') }}</h5>
                                        ...
                                        <button id="whatsapp_disconnect_btn" type="button" class="btn btn-danger" style="display:none;">{{ __('Disconnect WhatsApp') }}</button>
                                    </div>
                                </div>
                            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function initWhatsAppSocket() {
            ...
            const userId = "school_{{ Auth::user()->school_id }}";
