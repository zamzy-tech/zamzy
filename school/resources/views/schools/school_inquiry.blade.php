                ...
                <form id="create-form" class="pt-3 school-registration-form" action="{{ route('school-inquiry.update') }}">
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                <label>{{ __('DOMAIN TYPE') }} <span class="text-danger">*</span></label><br>
                                <div class="d-flex">
                                    <div class="form-check form-check-inline">
                                        <label class="form-check-label">
                                            {!! Form::radio('domain_type', 'default', true, ['id' => 'edit_domain_type_default', 'class' => 'edit_domain_type']) !!}
                                            {{ __('DEFAULT') }}
                                        </label>
                                    </div>
                                    <div class="form-check form-check-inline ml-2">
                                        <label class="form-check-label">
                                            {!! Form::radio('domain_type', 'custom', false, ['id' => 'edit_domain_type_custom', 'class' => 'edit_domain_type']) !!}
                                            {{ __('CUSTOM') }}
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="form-group col-sm-12 col-md-6">
                                <label for="edit_domain" id="edit_domain_label">{{ __('sub_domain') }} <span class="text-danger">*</span></label>
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control domain-pattern" name="domain" id="edit_domain" placeholder="{{ __('domain') }}" aria-label="Recipient's username" aria-describedby="basic-addon2" required>
                                    <div class="input-group-append" id="edit_domain_addon_wrapper">
...
@section('js')
    <script>
        ...
        $(document).ready(function() {
            $(document).on('change', '.edit_domain_type', function() {
                let val = $(this).val();
                if (val === 'default') {
                    $('#edit_domain_label').html("{{ __('sub_domain') }} <span class='text-danger'>*</span>");
                    $('#edit_domain').attr('placeholder', "{{ __('sub_domain') }}");
                    $('#edit_domain_addon_wrapper').removeClass('d-none');
                } else {
                    $('#edit_domain_label').html("{{ __('custom_domain') }} <span class='text-danger'>*</span>");
                    $('#edit_domain').attr('placeholder', "{{ __('custom_domain') }}");
                    $('#edit_domain_addon_wrapper').addClass('d-none');
                }
            });
        });
    </script>
