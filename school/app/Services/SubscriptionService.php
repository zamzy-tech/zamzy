...
class SubscriptionService
{
    ...
    public function createSubscription($package_id, $school_id = null, $subscription_id = null, $isCurrentPlan = null)
    {
        ...
        if ($isCurrentPlan && $start_date <= Carbon::now()->format('Y-m-d')) {
            ...
            if ($package->type == 0) {
                $has_prior_premium_subscriptions = Subscription::where('school_id', $subscription->school_id)
                    ->where('id', '!=', $subscription->id)
                    ->whereHas('package', function($q) {
                        $q->where('is_trial', 0);
                    })->count() > 0;

                $bill_amount = !$has_prior_premium_subscriptions ? 1 : $package->charges;
    ...
    public function createSubscriptionBill($subscription, $generateBill = null)
    {
        ...
        if ($subscription->package_type == 1) {
            // Postpaid: Monthly per school total fixed amount only (pro-rated by usage days)
            $postpaid_charges = number_format((($usage_days * $subscription->charges) / $bill_cycle_days), 2);
            $bill_amount = $postpaid_charges + $addons;
        } else {
            // Prepaid (or fallback)
            ...
            $bill_amount = $student_charges + $staff_charges + $addons;
        }
    ...
    public function stripe_payment($subscriptionBill_id = null, $package_id = null, $type = null, $subscription_id = null, $isCurrentPlan = null)
    {
        try {
            ...
            if ($package_id && $package_id != -1) {
                ...
                $has_prior_premium_subscriptions = Subscription::where('school_id', $schoolId)
                    ->whereHas('package', function($q) {
                        $q->where('is_trial', 0);
                    })->count() > 0;

                if (!$has_prior_premium_subscriptions) {
                    $amount = 1; // 1 Month Free Trial for ₹1
                } else {
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_POSTFIELDS => http_build_query([
                    ...
                    'metadata[user_id]' => Auth::user()->id ?? null
                ])
            ]);
    ...
    public function activateSchool($school_id, $amount)
    {
        try {
            $school = \App\Models\School::on('mysql')->withTrashed()->find($school_id);
            if ($school) {
                if ($school->status == 0) {
                    $school->status = 1;
                    $school->save();
                    Log::info("School ID {$school_id} activated successfully.");
                }

                // Clear cache
                $this->cache->removeSchoolCache(config('constants.CACHE.SCHOOL.SETTINGS'), $school_id);
                ...
                if ($adminUser) {
                    $systemSettings = $this->cache->getSystemSettings();
                    $emailData = [
                        'subject' => 'Payment Successful & Account Activated - ' . ($systemSettings['system_name'] ?? 'eSchool'),
                        'email' => $adminUser->email,
                        'email_body' => 'Dear ' . $adminUser->first_name . ' ' . $adminUser->last_name . ",<br><br>Thank you! Your payment of ₹" . $amount . " has been successfully processed, and your school account has been activated.<br><br>You can now access your dashboard and configure your school: <a href=\"" . url('/') . "\">Go to Dashboard</a><br><br>Best regards,<br>" . ($systemSettings['system_name'] ?? 'eSchool Team')
                    ];
                    ...
                    Log::info("Activation email sent to school admin: " . $adminUser->email);
                } else {
                    Log::warning("No admin user found for school ID {$school_id} during activation.");
                }
            } else {
                Log::error("School not found with ID {$school_id} during activation.");
            }
        } catch (\Throwable $e) {
            Log::error("Error activating school or sending email: " . $e->getMessage());
        }
    }
}
