...
class TeacherController extends Controller {
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $staff = $this->staff->create([
                ...
                'salary'        => !empty($request->salary) ? $request->salary : 0,
            ...
            $userService->sendStaffRegistrationEmail($user, $request->mobile);

            if ($user->school_id) {
                $whatsapp_status = $this->cache->getSchoolSettings('whatsapp_status', $user->school_id);
                if ($whatsapp_status === '' || $whatsapp_status === null) {
                    $whatsapp_status = 1;
                }
                if ($whatsapp_status == 1 && !empty($user->mobile)) {
                    $userService->sendStaffRegistrationWhatsApp($user, $request->mobile);
                }
            }
    ...
    public function update(Request $request, $id) {
        ...
        try {
            ...
            //Call store function of User Repository and get the User Data
            $this->staff->update($user->staff->id, array('qualification' => $request->qualification, 'salary' => !empty($request->salary) ? $request->salary : 0,'joining_date'   => date('Y-m-d',strtotime($request->joining_date))));
