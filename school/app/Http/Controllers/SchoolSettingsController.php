...
class SchoolSettingsController extends Controller {
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            foreach ($settings as $key => $rule) {
                if ($key == 'horizontal_logo' || $key == 'vertical_logo' || $key == 'favicon' || $key == 'upi_qr_code') {
    ...
    public function emailTemplateUpdate(Request $request)
    {
        ...
        try {
            ...
            [
                'name' => 'email-template-payment',
                'data' => htmlspecialchars($request->payment_data),
                'type' => 'string'
            ],
            [
                'name' => 'email-template-absent',
                'data' => htmlspecialchars($request->absent_data),
                'type' => 'string'
            ],
            [
                'name' => 'email-template-leave',
                'data' => htmlspecialchars($request->leave_data),
                'type' => 'string'
            ]
        );
            $this->schoolSettings->upsert($OtherSettingsData, ["name"], ["data"]);
    ...
    {
        if (auth()->user()->school_id && !\App\Services\FeaturesService::hasAnyFeature(['Website Management', 'WhatsApp SMS'])) {
            return redirect()->back()->withErrors([
                'message' => trans('Purchase') . " " . trans('Website Management') . " " . trans('or') . " " . trans('WhatsApp SMS') . " " . trans("to Continue using this functionality")
            ])->send();
        }
        ResponseService::noPermissionThenRedirect('school-setting-manage');
    ...
    public function thirdPartyApiUpdate(Request $request)
    {
        ...
        try {
            ...
            [
                ...
                "data" => $request->input('whatsapp_status', 0),
                "type" => "text"
            ]);
    ...
    public function whatsappTemplate()
    {
        ResponseService::noAnyPermissionThenRedirect(['email-template']);
        $settings = $this->cache->getSchoolSettings();
        return view('school-settings.whatsapp_template', compact('settings'));
    }
    ...
    public function whatsappTemplateUpdate(Request $request)
    {
        ...
        try {
            $OtherSettingsData = array([
                'name' => 'whatsapp-template-staff',
                'data' => htmlspecialchars($request->staff_data),
                'type' => 'string',
            ],
            [
                'name' => 'whatsapp-template-parent',
                'data' => htmlspecialchars($request->parent_data),
                'type' => 'string'
            ],
            [
                'name' => 'whatsapp-template-fee-payment',
                'data' => htmlspecialchars($request->fee_payment_data),
                'type' => 'string'
            ],
            [
                'name' => 'whatsapp-template-absent',
                'data' => htmlspecialchars($request->absent_data),
                'type' => 'string'
            ],
            [
                'name' => 'whatsapp-template-leave',
                'data' => htmlspecialchars($request->leave_data),
                'type' => 'string'
            ]
        );
            ...
            ResponseService::logErrorResponse($e, "School Settings Controller -> whatsappTemplateUpdate method");
            ResponseService::errorResponse();
        }
    }
}
