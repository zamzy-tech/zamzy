...
class Controller extends BaseController {
    ...
    public function systemLinks($type = null)
    {
        if ($type) {
            ...
            try {
                ...
                // Handle case where default connection has no database configured
            }
    ...
    public function registerStudent(Request $request)
    {
        ...
        try {
            ...
            $parentUser = null;
            if (!empty($request->guardian_email)) {
                $parentUser = User::whereHas('roles', function ($q) {
                    $q->where('name', 'Guardian');
                })->where('email', $request->guardian_email)->first();
            } else if (!empty($request->guardian_mobile)) {
                $parentUser = User::whereHas('roles', function ($q) {
                    $q->where('name', 'Guardian');
                })->where('mobile', $request->guardian_mobile)->first();
            }
            ...
            if (!empty($parentUser)) {
                ...
                $parent['email'] = !empty($request->guardian_email) ? $request->guardian_email : null;
            ...
            if ($request->hasFile('aadhar_pic')) {
                $aadhar_pic = UploadService::upload($request->aadhar_pic, 'students');
            }
    ...
    public function leadForm() {
        ...
        return view('lead-form', compact('systemSettings'));
    }
            ...
            try {
                ...
                if ($admin_email) {
                    $data = [
                        ...
                        'description' => "New Lead from Google Ads:\n\nName: {$request->name}\nPhone: {$request->phone}\nEmail: {$request->email}\nSchool: {$request->school_name}\nCity: {$request->city}\nStudents: {$request->students_count}\nMessage: {$request->message}",
                        'admin_email' => $admin_email,
                    ];
                    Mail::send('contact', $data, static function ($message) use ($data) {
                        $message->to($data['admin_email'])->subject('New Lead - Demo Request');
                    });
                }
            } catch (\Throwable $e) {
                // Silently fail email - lead is already saved
            }
