...
class SubscriptionWebhookController extends Controller
{
    ...
    public function razorpay(Request $request)
    {
        ...
        try {
            ...
            if ($metadata && isset($data->event) && $data->event == 'payment.captured') {
                ...
                if ($metadata->type == 'package') {
                    $this->subscriptionService->activateSchool($metadata->school_id, $metadata->amount);
                }
    ...
    private function handlePackagePayment($metadata)
    {
        ...
        try {
            ...
            $amount = $metadata['amount'] ?? $metadata['price'] ?? 0;
            $this->subscriptionService->activateSchool($schoolId, $amount);
