...
class SubscriptionController extends Controller
{
    ...
    public function index()
    {
        ...
        $school_database_name = \Illuminate\Support\Facades\Session::get('school_database_name');
        if ($school_database_name) {
            \App\Http\Middleware\SwitchDatabase::switchToSchool($school_database_name);
        } else {
            \App\Http\Middleware\SwitchDatabase::switchToMysql();
        }
    ...
    public function payment_success($check_out_session_id, $subscriptionBill_id = null, $package_id = null, $type = null, $subscription_id = null, $isCurrentPlan = null)
    {
        ...
        if (!$user) {
            ...
            if ($user_id) {
                $user = User::on('mysql')->withTrashed()->find($user_id);
                if ($user) {
                    Auth::login($user);
                }
            }
        ...
        if (!Auth::user()) {
            return redirect()->route('login')->withErrors(['email' => trans('session_expired')]);
        }
        $status = "pending";
        ...
        if ($type != -1) {
            ...
            if ($subscription->package_type == 1) {
                ...
                // Prepaid
                $has_prior_premium_subscriptions = \App\Models\Subscription::where('school_id', $subscription->school_id)
                    ->where('id', '!=', $subscription->id)
                    ->whereHas('package', function($q) {
                        $q->where('is_trial', 0);
                    })->count() > 0;

                $bill_amount = !$has_prior_premium_subscriptions ? 1 : $subscription->charges;
        ...
        $this->subscriptionService->activateSchool($subscriptionBill->school_id, $paymentTransaction->amount);

        $user = Auth::user();
        $school = \App\Models\School::on('mysql')->withTrashed()->find($user->school_id);
        if ($school) {
            if (empty($school->domain)) {
                $domain = \Illuminate\Support\Str::slug($school->name, '-');
                $originalDomain = $domain;
                $i = 1;
                while (\App\Models\School::on('mysql')->where('domain', $domain)->exists()) {
                    $domain = $originalDomain . '-' . $i;
                    $i++;
                }
                $school->domain = $domain;
                $school->domain_type = 'default';
                $school->save();
            }
            ...
            if ($school->domain_type == 'default') {
                $school_url = 'https://' . $school->domain . '.' . $baseUrlWithoutScheme;
            } else {
                $school_url = 'https://' . $school->domain;
            }

            \Illuminate\Support\Facades\URL::forceRootUrl($school_url);
            ...
            return redirect($signedUrl)->with('success', trans('the_payment_has_been_completed_successfully'));
        }
    ...
    public function razorpay_order_id(Request $request)
    {
        ...
        DB::setDefaultConnection('mysql');
        $paymentConfiguration = PaymentConfiguration::on('mysql')->whereNull('school_id')->where(['payment_method' => 'Razorpay'])->where('status',1)->first();
        ...
        if ($request->type == 'package') {
            $has_prior_premium_subscriptions = \App\Models\Subscription::where('school_id', $schoolId)
                ->whereHas('package', function($q) {
                    $q->where('is_trial', 0);
                })->count() > 0;
            if (!$has_prior_premium_subscriptions) {
                $checkout_amount = 1; // 1 Month Free Trial for ₹1
            }
        }
        ...
        $amount = max(100, round($checkout_amount * 100)); // Ensure minimum ₹1.00 (100 paise)
    ...
    {
        try {
            \Log::info('Razorpay Callback Request Parameters: ' . json_encode($request->all()));
            ...
            // Update payment transaction with order and payment IDs
            $paymentTransaction = PaymentTransaction::on('mysql')->find($request->paymentTransactionId);
            ...
            $paymentTransactionId = $paymentTransaction ? $paymentTransaction->id : 0;
            ...
            // Verify Razorpay payment signature locally
            $attributes = [
            ...
            // Directly handle subscription creation and school activation after payment
            // Don't rely solely on webhook which may fail
            try {
                $subscriptionService = app(\App\Services\SubscriptionService::class);
                
                if ($request->type == 'package' || $request->type == 'Package') {
                    $packageType = $request->package_type ?? 'new';
                    ...
                    // Activate the school
                    $subscriptionService->activateSchool($schoolId, $request->amount);
                    
                } elseif ($request->type == 'addon' || $request->type == 'Addon') {
                    $addonData = [
                        'subscription_id' => $request->subscription_id ?? 0,
                        'school_id' => $schoolId,
                        'feature_id' => $request->feature_id ?? '',
                    ...
                    \App\Models\AddonSubscription::create($addonData);
                }
                ...
                \Log::error('Razorpay post-payment processing error: ' . $subError->getMessage() . ' at ' . $subError->getFile() . ':' . $subError->getLine());
            }
            ...
            \Log::error('Razorpay payment error: ' . $th->getMessage() . ' at ' . $th->getFile() . ':' . $th->getLine());
            return $th;
        }
    }
