...
try {
    ...
    echo "Total Central: " . count($schools) . "\n";
    foreach ($schools as $s) {
        $dbname = str_replace('shacartc_eschool_saas_', '[P]_', $s['database_name'] ?? '');
        $del = $s['deleted_at'] ? "Y" : "N";
        echo "id:{$s['id']} | c:{$s['code']} | e:{$s['support_email']} | db:$dbname | s:{$s['status']} | d:$del\n";
    }
