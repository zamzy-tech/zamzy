...
select2Search($(".guardian-search"), baseUrl + "/guardian/search", null, 'Search for Guardian Email', Select2SearchDesignTemplate, function (repo) {
    if (!repo.text) {
        ...
        if (repo.id && repo.text !== 'Search for Guardian Email') {
            $('.guardian_email').val(repo.text).prop('readonly', false);
        } else {
            $('.guardian_email').val('').prop('readonly', false);
        }
    ...
    return repo.email || repo.mobile || repo.text;
});
...
$(document).on('submit', '.school-email-template', function (e) {
    ...
    let payment_data = formData.get('payment_data');
    let absent_data = formData.get('absent_data');
    let leave_data = formData.get('leave_data');
    ...
    $.ajax({
        ...
        data: {
            ...
            reject_email_data: reject_email_data,
...
$(document).on('submit', '.school-whatsapp-template', function (e) {
    e.preventDefault();
    let formData = new FormData(this);
    let staff_data = formData.get('staff_data');
    let parent_data = formData.get('parent_data');
    let fee_payment_data = formData.get('fee_payment_data');
    let absent_data = formData.get('absent_data');
    let leave_data = formData.get('leave_data');
    let template = formData.get('template');
    let submitButtonElement = $(this).find(':submit');
    ...
    $.ajax({
        ...
        data: {
            staff_data: staff_data,
            parent_data: parent_data,
            fee_payment_data: fee_payment_data,
            absent_data: absent_data,
            leave_data: leave_data,
            template: template
        },
