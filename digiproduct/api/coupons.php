...
$totalAmount = (int)($input['totalAmount'] ?? ($_GET['totalAmount'] ?? 0));
...
if (file_exists($dbFile)) {
    try {
        ...
        try {
            $stmt = $pdo->prepare("
                SELECT * FROM coupons 
                WHERE (UPPER(coupon_code) = :code OR UPPER(code) = :code) 
                  AND (active = 1 OR is_active = 1) 
                LIMIT 1
            ");
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT * FROM coupons WHERE UPPER(code) = :code AND (is_active = 1 OR active = 1) LIMIT 1");
                $stmt->execute([':code' => $code]);
                $c = $stmt->fetch(PDO::FETCH_ASSOC);
            }
        }
...
if ($c) {
    ...
    $type = strtoupper($c['discount_type'] ?? 'PERCENTAGE');
