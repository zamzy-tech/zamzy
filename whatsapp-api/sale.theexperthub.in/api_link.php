    ...
    <div class="tab-panel active" id="dashboardPanel">
      ...
      <div style="background: rgba(15,23,42,0.25); border: 1px solid rgba(255,255,255,0.06); padding: 20px; border-radius: 16px; margin-bottom: 24px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 10px;">
          ...
          ?>
            <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px; background: rgba(15,23,42,0.4); padding: 8px 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.03);">
              <span style="font-size: 12px; font-weight: 600; color: #94a3b8;">Device #<?= $i ?> API Key:</span>
              <code id="clientApiKey_<?= $i ?>" style="font-family: monospace; font-size: 12.5px; color: #38bdf8; word-break: break-all;"><?= htmlspecialchars($slot_key) ?></code>
              <button class="btn-copy" onclick="copyClientKeySlot(<?= $i ?>)" style="padding: 4px 10px; font-size: 11px; background: rgba(56,189,248,0.1); border: 1px solid rgba(56,189,248,0.2); color: #38bdf8; border-radius: 6px; cursor: pointer; transition: all 0.2s;">📋 Copy Key</button>
            </div>
          <?php endfor; ?>
        </div>
...
function copyClientKeySlot(slot) {
  const keyText = document.getElementById('clientApiKey_' + slot).textContent;
  navigator.clipboard.writeText(keyText).then(() => {
    alert(`Slot #${slot} API Key copied successfully!`);
  }).catch(err => {
...
function pollStatus() {
  ...
  statusUrl += (statusUrl.indexOf('?') !== -1 ? '&' : '?') + 'session=' + encodeURIComponent(LOGIN_ID + sessionSuffix);
...
<div style="min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 24px; background-color: var(--bg-main);">
  <div class="login-card" style="width: 100%; max-width: 440px; background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 20px; padding: 40px; box-shadow: 0 30px 70px rgba(0,0,0,0.5); text-align: center;">
    <div class="logo-area" style="margin-bottom: 28px; text-align: center;">
      ...
      <div class="logo" style="font-size: 22px; font-weight: 700; color: var(--lime); font-family: var(--font-heading); letter-spacing: 1px;">THE EXPERT HUB</div>
