...
<header>
  <div class="logo-block">
    ...
    <div class="tagline"><?= htmlspecialchars($settings['company_tagline'] ?? '') ?></div>
  </div>
  <nav>
    <a href="index.php" class="active">Generate Invoice</a>
  ...
  <div class="container">
    ...
    <div class="grid-2" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px; margin-bottom: 24px;">
      ...
      <div class="card" style="background: var(--bg-surface); border: 1px solid var(--border-color); padding: 24px; border-radius: 16px;">
        ...
        <form method="POST" action="index.php" id="singleLeadForm" style="display: flex; flex-direction: column; gap: 16px;">
        ...
        <form method="POST" action="index.php" id="bulkLeadForm" enctype="multipart/form-data" style="display: none; flex-direction: column; gap: 16px;">
        ...
        .then(data => {
          if (data.success) {
            ...
            if (gatewayUrl) {
              ...
              .then(gwRes => {
                window.location.href = 'index.php?staff_success=' + encodeURIComponent('Lead status updated and WhatsApp message sent successfully!');
              })
              .catch(gwErr => {
                console.error(gwErr);
                window.location.href = 'index.php?staff_warning=' + encodeURIComponent('Status updated in DB, but WhatsApp message failed to deliver.');
              });
            } else {
              const waUrl = 'https://api.whatsapp.com/send?phone=' + phone + '&text=' + encodeURIComponent(msgText);
              window.open(waUrl, '_blank');
              window.location.href = 'index.php?staff_success=' + encodeURIComponent('Status updated in DB. WhatsApp link opened in a new tab.');
            }
          } else {
