    ...
    if ($action === 'add_client') {
        ...
        if (empty($client_name)) {
            ...
            if ($check_stmt->fetchColumn() > 0) {
                ...
                try {
                    $ins_stmt = $pdo->prepare("INSERT INTO api_keys (client_name, api_key, credits, status, client_phone, login_id, login_password, expiry_date) VALUES (?, ?, ?, 'active', ?, ?, ?, ?)");
                    $ins_stmt->execute([$client_name, $api_key, $credits, $client_phone, $login_id, $password_hash, $expiry_date]);
        ...
        if ($client_id > 0) {
            try {
                $upd_stmt = $pdo->prepare("UPDATE api_keys SET credits = ?, expiry_date = ? WHERE id = ?");
