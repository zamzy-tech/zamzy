...
try {
    ...
    if ($check_stmt->fetchColumn() > 0) {
        header('Location: index.php?error=' . urlencode("Invoice number '$invoice_number' already exists. Please use a unique invoice number."));
        exit;
    }
} catch (PDOException $e) {
    header('Location: index.php?error=' . urlencode("Database validation failed: " . $e->getMessage()));
    exit;
}
