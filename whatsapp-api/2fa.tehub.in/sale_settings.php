...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'add_chatbot_rule') {
        $active_tab = 'chatbot';
...
<header>
  <div class="logo-block" style="display:flex;align-items:center;gap:12px;">
    <img src="teh_logo.png" alt="THE EXPERT HUB" style="height:36px;width:auto;">
    <div>
      <div class="logo" style="font-size: 20px; font-weight: 700; color: var(--lime); letter-spacing: 0.5px;">THE EXPERT HUB</div>
      <div class="tagline" style="font-size: 10px; color: var(--text-muted); text-transform: uppercase;">Admin Invoice Control</div>
    </div>
  </div>
  <nav>
      ...
      <div id="whatsappTab" style="display: <?= $active_tab === 'whatsapp' ? 'block' : 'none' ?>;">
            ...
            <div style="display: grid; grid-template-columns: 280px 1fr; gap: 32px; align-items: center;">
              ...
              <div style="text-align: center;">
                ...
                <div style="margin-top: 12px; font-size: 12px; text-align: center;">
                  <form method="POST" action="settings.php?tab=whatsapp" style="display: inline;" onsubmit="return confirm('Are you sure you want to force reset the WhatsApp connection? This will clear all session files.');">
                    <input type="hidden" name="action" value="disconnect_whatsapp">
                    <button type="submit" style="background: none; border: none; color: #e53e3e; text-decoration: underline; font-weight: 600; cursor: pointer; padding: 0;">
        ...
        <div class="card">
          ...
          <form method="POST" action="settings.php?tab=whatsapp">
            ...
            <div id="gatewayConfigFields" style="display: <?= ($settings['whatsapp_gateway_type'] ?? 'browser') === 'gateway' ? 'block' : 'none' ?>; background: #f7fafc; border: 1px solid #cbd5e0; border-radius: 10px; padding: 20px; margin-bottom: 20px;">
              <div class="form-group">
        ...
        <div class="card" style="padding: 24px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px; margin-bottom: 24px;">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 12px;">
              <h3 style="color:#1e293b; margin-bottom: 6px; font-size:15px; font-weight: 700;">🟢 Chatbot Status</h3>
              <p style="font-size:12.5px; color:#64748b; margin-bottom: 16px;">Enable or disable the automated keyword replies globally for the admin number (+91 89392 20422).</p>
              
              <form method="POST" action="sale_settings.php?tab=chatbot" style="display:flex; justify-content:space-between; align-items:center; background:#fff; border: 1px solid #e2e8f0; padding: 12px; border-radius: 8px;">
                ...
                <div style="display: flex; flex-direction: column;">
                  <span style="font-size:13px; font-weight: 700; color: #1e293b;">Chatbot Integration</span>
                  <span style="font-size:12px; color: <?= intval($settings['chatbot_enabled'] ?? 1) ? '#10b981' : '#ef4444' ?>; font-weight: 600; margin-top: 2px;">
                    ● <?= intval($settings['chatbot_enabled'] ?? 1) ? 'Currently Active' : 'Currently Inactive' ?>
                  </span>
                </div>
                
                <button type="submit" style="margin: 0; padding: 6px 12px; font-size: 12px; background: <?= intval($settings['chatbot_enabled'] ?? 1) ? '#ef4444' : '#10b981' ?>; color: #fff; font-weight: 700; border-radius: 6px; cursor: pointer; border: none; font-family: inherit;">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 12px;">
              <h3 style="color:#1e293b; margin-bottom: 6px; font-size:15px; font-weight: 700;">➕ Create New Rule</h3>
              <p style="font-size:12.5px; color:#64748b; margin-bottom: 16px;">Define a new automatic response for a specific trigger word or phrase.</p>
              
              <form method="POST" action="sale_settings.php?tab=chatbot">
                <input type="hidden" name="action" value="add_chatbot_rule">
                
                <div class="form-group" style="margin-bottom: 12px;">
                  <label style="font-weight:700; color:#334155; font-size:11.5px;">Trigger Keyword *</label>
                  <input type="text" name="keyword" placeholder="e.g. price" required style="width:100%; padding:8px 12px; border:1px solid #cbd5e0; border-radius:6px; font-size:13px; font-family:inherit;">
                </div>
                
                <div class="form-group" style="margin-bottom: 16px;">
                <div class="form-group" style="margin-bottom: 12px;">
                  <label style="font-weight:700; color:#334155; font-size:11.5px;">Reply Message Text *</label>
                  <textarea name="reply_text" rows="3" placeholder="Enter auto-reply text here..." required style="width:100%; padding:8px 12px; border:1px solid #cbd5e0; border-radius:6px; font-size:13px; font-family:inherit; resize:vertical;"></textarea>
                </div>

                <div class="form-group" style="margin-bottom: 12px;">
                  <label style="font-weight:700; color:#334155; font-size:11.5px;">Image URL (Optional)</label>
                  <input type="url" name="image_url" placeholder="https://example.com/banner.jpg" style="width:100%; padding:8px 12px; border:1px solid #cbd5e0; border-radius:6px; font-size:13px; font-family:inherit;">
                </div>

                <div class="form-group" style="margin-bottom: 16px;">
                  <label style="font-weight:700; color:#334155; font-size:11.5px;">Chatbot Buttons (Optional, comma-separated)</label>
                  <input type="text" name="buttons_list" placeholder="e.g. Services, Sales, Website|https://tehub.in" style="width:100%; padding:8px 12px; border:1px solid #cbd5e0; border-radius:6px; font-size:13px; font-family:inherit;">
                  <span style="font-size:10.5px; color:#64748b; margin-top:2px;">Format: <code>ButtonName</code> or <code>ButtonName|LinkURL</code> for website link buttons. Max 3 buttons.</span>
                </div>
                
                <button type="submit" class="btn-save" style="margin: 0; width: 100%; border-radius: 6px;">Add Rule</button>
              </form>
            </div>
          </div>
          ...
          <div style="margin-top: 16px;">
            <h3 style="color:#1e293b; margin-bottom: 12px; font-size:15px; font-weight: 700; border-bottom:1px solid #e2e8f0; padding-bottom: 8px;">📋 Active Chatbot Rules</h3>
            
            <?php if (empty($chatbot_rules)): ?>
              <div style="text-align: center; padding: 32px; background: #f8fafc; border-radius: 12px; border: 1px dashed #cbd5e0; color: #64748b; font-size: 13.5px;">
            ...
            <?php else: ?>
              <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; font-size: 13px; text-align: left;">
                  <thead>
                    <tr style="background: #f1f5f9; color: #334155; font-weight: 700; border-bottom: 2px solid #cbd5e0;">
                      <th style="padding: 10px 14px;">Trigger Keyword</th>
                      <th style="padding: 10px 14px;">Auto-Reply Message</th>
                      <th style="padding: 10px 14px; width: 120px;">Image Header</th>
                      <th style="padding: 10px 14px; text-align: center; width: 80px;">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($chatbot_rules as $r): ?>
                      <tr style="border-bottom: 1px solid #e2e8f0; vertical-align: top;">
                        <td style="padding: 12px 14px; font-weight: 700; color: #0f172a; white-space: nowrap;">
                          <code style="background:#f1f5f9; padding: 2px 6px; border-radius: 4px; border:1px solid #cbd5e0; font-family:var(--font-mono);"><?= htmlspecialchars($r['keyword']) ?></code>
                        </td>
                        <td style="padding: 12px 14px; color: #334155; line-height: 1.4; white-space: pre-line;">
                          ...
                          <?php if (!empty($buttons_display)): ?>
                            <div style="font-size: 11px; margin-top: 6px; color: #10b981; font-weight: 700; background: #ecfdf5; padding: 4px 8px; border-radius: 6px; border: 1px dashed #a7f3d0; display: inline-block;">
                        ...
                        <td style="padding: 12px 14px; vertical-align: middle;">
                          <?php if (!empty($r['image_url'])): ?>
                            <a href="<?= htmlspecialchars($r['image_url']) ?>" target="_blank" style="display:flex; align-items:center; gap:6px; text-decoration:none; color:#3182ce; font-weight:600;">
                              <img src="<?= htmlspecialchars($r['image_url']) ?>" style="width:32px; height:32px; object-fit:cover; border-radius:4px; border:1px solid #cbd5e0;" onerror="this.src='https://placehold.co/80x80?text=Img';">
                        ...
                        <td style="padding: 12px 14px; text-align: center; vertical-align: middle;">
                          <form method="POST" action="sale_settings.php?tab=chatbot" onsubmit="return confirm('Are you sure you want to delete this chatbot rule?');">
                            <input type="hidden" name="action" value="delete_chatbot_rule">
                            <input type="hidden" name="rule_id" value="<?= $r['id'] ?>">
                            <button type="submit" class="btn-action-small delete" style="padding: 4px 8px; font-size: 11px; background:#fee2e2; border:1px solid #fecaca; color:#ef4444; border-radius: 4px; font-weight:600; cursor:pointer;">Delete</button>
                          </form>
                        </td>
                      </tr>
                    <?php endforeach; ?>
...
function pollWhatsAppStatus() {
  ...
  let statusUrl = 'http://localhost:3000/status?session=default&t=' + Date.now();
  if (typeof GATEWAY_URL !== 'undefined' && GATEWAY_URL) {
    statusUrl = GATEWAY_URL.replace('/send', '/status');
    statusUrl += (statusUrl.indexOf('?') !== -1 ? '&' : '?') + 'session=default&t=' + Date.now();
  }
